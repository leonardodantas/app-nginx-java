...
public class SearchProductController {
    ...
    public List<ProductResponse> searchByCategoriesV1(@RequestBody final CategoriesRequest request) {
        final var response = searchProduct.findByCategoriesV1(request.values());
        return response.stream().map(ProductResponse::from).toList();
    }

    @GetMapping("v2/categories")
    @ResponseStatus(HttpStatus.OK)
    @Operation(summary = "Busca de uma lista de produtos por uma lista de categorias no PostgresSQL")
    public List<ProductResponse> searchByCategoriesV2(@RequestBody final CategoriesRequest request) {
        final var response = searchProduct.findByCategoriesV2(request.values());
        return response.stream().map(ProductResponse::from).toList();
    }
}
